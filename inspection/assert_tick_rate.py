...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("Usage: python inspection/assert_tick_rate.py <tick_observation.jsonl>", file=sys.stderr)
        return 1
    ...
    for asset in sorted(buckets):
        counts = list(buckets[asset].values())
        print(f"asset: {asset}")
        print(f"  minutes_observed: {len(counts)}")
        print(f"  mean_ticks_per_minute: {sum(counts)/len(counts):.3f}")
