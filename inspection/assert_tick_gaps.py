...
Descriptive inspection only. No enforcement.
"""
...
def main(argv: list[str]) -> int:
    ...
    for asset in sorted(counts):
        g = gaps.get(asset, [])
        print(f"asset: {asset}")
        print(f"  total_ticks: {counts[asset]}")
        print(f"  max_gap_s: {max(g):.3f}" if g else "  max_gap_s: n/a")
        print(f"  p95_gap_s: {_p95(g):.3f}" if g else "  p95_gap_s: n/a")
        print(f"  gaps_over_{threshold:g}s: {sum(1 for x in g if x > threshold)}")
